const toggleMenu = () => {
  ...
  document.addEventListener('click', (event) =>{
    ...
    if(currentClick.matches('.callback-btn, .modal-overlay, .button-services, .before') || currentClick.closest('.modal-close')){
      handlerMenu();
    } 
  ...
  for(let i = 0; i < wrapper.length; i++){
    if(wrapper[i].closest('.col-sm-6')) {
      const before = document.createElement('div');
      before.className = 'before';
      before.textContent = 'Оформить заявку';
      wrapper[i].appendChild(before);
    }
  }
